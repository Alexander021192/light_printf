...
/*   Updated: 2021/03/16 17:59:40 by ocalamar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_init_textures(t_all *all)
{
	...
	while(i < 5)
	{
		...
		&all->tex[i].width, &all->tex[i].height)))
			all->tex[i].ptr = mlx_get_data_addr(all->tex[i].tex, &all->tex[i].bpp, 
			&all->tex[i].size_line, &all->tex[i].endian);
		else
...
void	ft_init_map_size(t_all *all)
{
	...
	all->win_width += all->win_width % 64; //hello my kostil
...
int		ft_draw_back(t_all *all)
{
	...
	while(sky_floor.y < all->win_height / 2)
	{
		...
		while(sky_floor.x < all->win_width)
			my_mlx_pixel_put(&all->screen, sky_floor.x++, sky_floor.y, all->ceil_color);
		sky_floor.y++;
	}
	while(sky_floor.y < all->win_height)
	{
		sky_floor.x = 0;
		while(sky_floor.x < all->win_width)
			my_mlx_pixel_put(&all->screen, sky_floor.x++, sky_floor.y, all->floor_color);
		sky_floor.y++;
	}
	return (0);
}
	...
	while (i++ < sprite->size.x)
	{
		...
		while (j++ < sprite->size.y - 2)
		{
			if(v_offset + j < 0 || v_offset + j >= all->win_height - 5)
				continue;
			int color = get_tex_color(&all->tex[4], 
			i*all->tex[4].width/sprite->size.x, j*all->tex[4].height/sprite->size.y);
			if(color)
